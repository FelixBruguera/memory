const Scores = ({ level, score, bestScore }) => {
    return (
        <div id="scores">
            <button className={`header-item red-hover ${level > 1 ? "text-transition" : null}`} key={level}>
            <svg className="button-icon" xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#e8eaed"><path d="m140-220-60-60 300-300 160 160 284-320 56 56-340 384-160-160-240 240Z"/></svg>
                Level: {level}</button>
            <button className={`header-item yellow-hover ${score > 0 ? "text-transition" : null}`} key={score}>
            ...
            <button className="header-item red-hover">
